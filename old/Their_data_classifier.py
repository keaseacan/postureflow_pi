...
# === Load Data ===
df = pd.read_excel("BreathBase_features copy.xlsx")
df["Person"] = df["Person"].astype(str).str.strip()

# === Speaker-wise IMF normalization ===
imf_cols = [f"IMF_{i}" for i in range(1, 10)]
df[imf_cols] = df.groupby("Person")[imf_cols].transform(lambda x: (x - x.mean()) / x.std())
...
# === Group-aware train/test split (by speaker) ===
groups = df["Person"]
...
plt.title("Speaker-Normalized + Auto-Weighted Voting Confusion Matrix")
